...
/** SWR sensitivity table — shows corpus for different withdrawal rates */
function buildSWRSensitivity(
  expenses: number,
  swrRates: number[]
): { swr: string; corpus: number; monthly: number }[] {
  return swrRates.map((r) => ({
    swr: `${r}%`,
    corpus: Math.round(expenses / (r / 100)),
...
export default function FIRECalculator() {
  ...
  const calc = useMemo((): CalcResult => {
    ...
    const fireTarget = swr > 0 ? expenses / swr : 0;
    const leanFireTarget = swr > 0 ? (expenses * 0.7) / swr : 0;
    const fatFireTarget = swr > 0 ? (expenses * 2.5) / swr : 0;

    const yearsToFire = yearsToReach(currentSavings, fireTarget, annualSavings, returnRate);
    const fireAge = yearsToFire === Infinity ? null : currentAge + yearsToFire;

    const pctOfFire =
      fireTarget > 0 ? Math.min(100, Math.round((currentSavings / fireTarget) * 100)) : 0;

    const savingsRate = annualIncome > 0 ? (annualSavings / annualIncome) * 100 : 0;
    const monthlyExpenses = Math.round(expenses / 12);
    const monthlyPassiveIncome = Math.round((currentSavings * (swr)) / 12);

    const swrSensitivity = buildSWRSensitivity(expenses, [2.5, 3, 3.5, 4, 4.5, 5]);

    const chartYears = Math.min(60, (yearsToFire === Infinity ? 40 : yearsToFire) + 5);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex justify-between text-xs mt-1">
                  ...
                  <span className="font-bold text-orange-600">{formData.swr}%</span>
                  <span className="text-slate-400">6% (bold)</span>
                </div>
                <p className="text-xs text-slate-500 mt-1">
                  The classic &quot;4% rule&quot; (25× expenses) is the most common starting point.
                </p>
              </div>
              <div>
                <label className="block text-sm sm:text-base font-bold text-slate-800 mb-2">
                ...
                <div className="flex justify-between text-xs mt-1">
                  ...
                  <span className="font-bold text-amber-600">{formData.returnRate}% p.a.</span>
          ...
          {showResult && (
            <div className="mt-8 space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="bg-gradient-to-br from-orange-500 to-amber-500 text-white rounded-2xl shadow-lg p-6">
                ...
                <p className="text-4xl sm:text-5xl font-black tracking-tight">{fmtL(calc.fireTarget)}</p>
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  <div className="bg-white/20 rounded-xl px-4 py-2 text-center">
                    ...
                    <p className="font-black text-base">{fmt(calc.monthlyExpenses)}</p>
                  </div>
                  <div className="bg-white/20 rounded-xl px-4 py-2 text-center">
                    <p className="text-xs text-white/70">Portfolio covers/mo now</p>
                    <p className="font-black text-base">{fmt(calc.monthlyPassiveIncome)}</p>
                  </div>
                  <div className="bg-white/20 rounded-xl px-4 py-2 text-center">
                    <p className="text-xs text-white/70">Savings rate</p>
                    <p className="font-black text-base">{calc.savingsRate.toFixed(1)}%</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                ...
                <div className="w-full h-5 bg-slate-100 rounded-full overflow-hidden">
                  ...
                  >
                    {calc.pctOfFire >= 15 && (
                      <span className="text-white text-[10px] font-black">{calc.pctOfFire}%</span>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <StatCard
                  ...
                  sub={calc.savingsRate >= 50 ? "🔥 Excellent!" : calc.savingsRate >= 30 ? "👍 Good" : "⬆️ Increase this"}
                />
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                <p className="text-sm font-bold text-slate-700 mb-1">Your FIRE Spectrum</p>
                <p className="text-xs text-slate-400 mb-4">Lean vs Standard vs Fat FIRE corpus comparison</p>
                <div className="h-44">
                ...
                <div className="flex gap-4 mt-2 flex-wrap text-xs">
                  <span className="flex items-center gap-1.5 text-emerald-600 font-medium">
                    <div className="w-2 h-2 rounded-full bg-emerald-400" /> Lean {fmtL(calc.leanFireTarget)}
                  </span>
                  <span className="flex items-center gap-1.5 text-orange-600 font-medium">
                    <div className="w-2 h-2 rounded-full bg-orange-400" /> Standard {fmtL(calc.fireTarget)}
                  </span>
                  <span className="flex items-center gap-1.5 text-violet-600 font-medium">
                    <div className="w-2 h-2 rounded-full bg-violet-500" /> Fat {fmtL(calc.fatFireTarget)}
                  </span>
                </div>
              </div>
                ...
                {showSensitivity && (
                  <div className="mt-3 bg-white border border-slate-100 rounded-2xl p-5 shadow-sm overflow-x-auto">
                    <p className="text-xs text-slate-400 mb-3">
                      How your FIRE corpus changes with different withdrawal rates
                    </p>
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-100">
                          <th className="text-left font-bold text-slate-600 pb-2 pr-4">SWR</th>
                          <th className="text-left font-bold text-slate-600 pb-2 pr-4">Corpus Needed</th>
                          <th className="text-left font-bold text-slate-600 pb-2">Rule</th>
                        </tr>
                      </thead>
                      <tbody>
                        {calc.swrSensitivity.map((row) => {
                          ...
                          return (
                            ...
                            >
                              <td className={`py-2 pr-4 font-bold ${isSelected ? "text-orange-600" : "text-slate-700"}`}>
                              ...
                              <td className="py-2 text-slate-500">
                                {(100 / parseFloat(row.swr)).toFixed(0)}× expenses
                              </td>
                            </tr>
                          );
                        })}
              ...
              <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl">
                <p className="text-sm text-orange-800 leading-relaxed">
                  ...
                  <strong>{formData.returnRate}%</strong> returns. Your portfolio hits{" "}
                  <strong>{fmtL(calc.fireTarget)}</strong>
                  {calc.fireAge ? ` at age ${calc.fireAge}` : ""}. Then withdraw{" "}
                  <strong>{formData.swr}%</strong> — covering all your expenses forever,
                  with high probability your money outlives you.
                </p>
              </div>
          ...
          <div className="mt-8 bg-slate-50 border border-slate-200 px-4 sm:px-5 py-3 sm:py-4 rounded-xl">
            <div className="flex items-start gap-3">
              ...
              <div className="text-xs text-slate-600 leading-relaxed space-y-1">
                ...
                <p>
                  FIRE Number = Annual Expenses ÷ SWR. The famous &quot;4% rule&quot; means 25× your
                  expenses. At that corpus, a diversified portfolio has historically supported
                  30+ year withdrawals with high success rates.
                </p>
                <p>
                  Timeline uses compound growth: <em>FV = PV × (1+r)ⁿ + PMT × ((1+r)ⁿ − 1) / r</em>
                </p>
              </div>
            </div>
          </div>
    ...
    return (
      <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
        <p className="text-sm text-emerald-800 font-bold">
          🎉 You&apos;ve hit your FIRE number! Financial independence is yours right now.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-xl">
        <p className="text-sm text-red-800 font-bold">
          ⚠️ FIRE is not achievable at your current savings rate. Increasing your
          savings rate is the single highest-impact lever you can pull.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl">
        <p className="text-sm text-orange-800 font-bold">
          🔥 Exceptional savings rate of <strong>{calc.savingsRate.toFixed(1)}%</strong>!
          You&apos;re on an accelerated FIRE path — reaching financial independence at age{" "}
